...
$upload = $_FILES['eventshowimg'];  //展示圖片
  ...
  if ($upload['error'] == 0) {
    ...
    $newsID = $_POST["nID"];
    $result = $db_link->query("SELECT newsimg FROM news WHERE newsID = $newsID");
    $row_result = $result->fetch_assoc();
    $filename = $row_result["newsimg"];
    //upload failure
    echo "不更動圖片";
  }
...
$sql_update = "UPDATE news SET newstitle=?, newscontent=?, newsdate=?, newstatus=?, newsimg=?, newsother=? WHERE newsID = ?";
$stmt = $db_link->prepare($sql_update);
$stmt->bind_param("sssissi", $newstitle, $newscontent, $newsdate, $newstatus, $filename, $newsother, $newsID);
$stmt->execute();
$stmt->close();
